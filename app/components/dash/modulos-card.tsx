...
import { useRetrieveUserQuery, useLogoutMutation } from "@/redux/features/auth/authApiSlice";
...
export default function ModulosGrid() {
  ...
  const getModuleConfig = (nombreModulo: string) => {
    const isCapacitacion = nombreModulo === "Capactiación" || nombreModulo === "Capacitación";
    if (isCapacitacion) {
      return {
        title: "Capacitación",
        desc: "Gestiona cursos, contenidos y rutas de aprendizaje.",
        icon: TbBook,
        iconName: "book-open",
        theme: {
          bgBlob: "from-blue-400/20 to-indigo-500/10",
          iconColor: "text-[#0056D2]",
          iconBg: "bg-blue-50/85 border-blue-100/50 shadow-[0_8px_20px_-4px_rgba(37,99,235,0.06)]",
          arrowBg: "bg-blue-50 text-blue-600",
        }
      };
    } else {
      return {
        title: "Configuración",
        desc: "Personaliza la plataforma, roles, permisos y notificaciones.",
        icon: TbSettings,
        iconName: "settings",
        theme: {
          bgBlob: "from-teal-400/20 to-emerald-500/10",
          iconColor: "text-[#0d9488]",
          iconBg: "bg-teal-50/85 border-teal-100/50 shadow-[0_8px_20px_-4px_rgba(13,148,136,0.06)]",
          arrowBg: "bg-blue-50 text-blue-600",
        }
      };
    }
      ...
      <div className="flex-1 p-8 lg:p-12 flex flex-col justify-center items-center relative overflow-y-auto h-full bg-[#f8fafc]">
        ...
        <div className="absolute inset-0 pointer-events-none select-none z-0">
          <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 800" preserveAspectRatio="none">
            <defs>
              <linearGradient id="waveBlueOnly" x1="0%" y1="0%" x2="100%" y2="0%">
            ...
            <path d="M-100,0 L-100,280 C200,380 600,160 900,280 C1200,400 1300,240 1600,320 L1600,0 Z" fill="url(#fadeBlue)" />
            
            {/* Brillos radiales sutiles */}
            <circle cx="200" cy="200" r="250" fill="#3b82f6" fillOpacity="0.02" filter="blur(70px)" />
            <circle cx="1200" cy="600" r="300" fill="#10b981" fillOpacity="0.01" filter="blur(90px)" />
          </svg>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center gap-10 lg:gap-12 relative z-10 w-full max-w-5xl">
          {user?.modulos_accesibles?.map((m) => {
            const config = getModuleConfig(m.nombre);
              ...
              >
                ...
                <div className="flex flex-col items-center mt-4 w-full relative">
                  ...
                  <div className="relative flex items-center justify-center mb-8">
                    ...
                    <div className={`absolute w-44 h-16 rounded-full blur-xl bg-gradient-to-r ${config.theme.bgBlob} opacity-80 -left-10 top-4 -rotate-[18deg] z-0 pointer-events-none group-hover:scale-110 transition-transform duration-500`} />
                    
                    {/* Círculo del Icono (Redondo, con fondo claro) */}
                    <div className={`w-28 h-28 rounded-full flex items-center justify-center border z-10 group-hover:scale-105 transition-transform duration-500 relative ${config.theme.iconBg}`}>
                      <IconComponent size={52} className={config.theme.iconColor} strokeWidth={1.0} />
                    </div>
                  </div>
                ...
                <div className="absolute bottom-8 right-8 w-12 h-12 rounded-full bg-[#f0f4ff] text-[#0056D2] flex items-center justify-center group-hover:bg-[#1c2634] group-hover:text-white transition-all duration-300 shadow-sm group-hover:shadow-md">
                  <ArrowUpRight className="w-5.5 h-5.5 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform duration-300" strokeWidth={2.5} />
                </div>
              </Link>
            );
          })}
